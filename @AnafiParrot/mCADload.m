...
% Load ANAFI Parrot 3D CAD model

uav.pCAD.obj{1} = loadawobj('Parrot_ANAFI.obj');
uav.pCAD.mtl{1} = loadawmtl('Parrot_ANAFI.mtl');
...
for ii = 1:3
    uav.pCAD.obj{1}.v(ii,:) = (uav.pCAD.obj{1}.v(ii,:) - mExt(ii))/1e6; % Era /1000, mas fica muito pequeno...
end

%------------------------------------------------
uav.pCAD.obj{2} = loadawobj('Parrot_ANAFI_body.obj');
uav.pCAD.mtl{2} = loadawmtl('Parrot_ANAFI_body.mtl');

uav.pCAD.obj{3} = loadawobj('Parrot_ANAFI_helice.obj');
uav.pCAD.mtl{3} = loadawmtl('Parrot_ANAFI_helice.mtl');
...
if num > 1
    for i = 2:num
        ...
        for ii = 1:3
                uav.pCAD.obj{i}.v(ii,:) = (uav.pCAD.obj{i}.v(ii,:) - mExt(ii))/1e6;
        end


        if i == 3
            phi = -pi/80;
            uav.pCAD.obj{i}.v = [1 0 0; 0 cos(phi) -sin(phi); 0 sin(phi) cos(phi)]*uav.pCAD.obj{i}.v;                      
            phi = pi/60;
            uav.pCAD.obj{i}.v = [cos(phi) 0 -sin(phi); 0 1 0; sin(phi) 0 cos(phi)]*uav.pCAD.obj{i}.v;   
